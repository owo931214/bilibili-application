            ...
            for data in datas:
                match data['cmd']:
                    case 'COMBO_SEND':
                        pass # TODO
                    case 'DANMU_MSG':
                        danmu_msg = {"msg":data['info'][1], "time":data['info'][0][5], "send_by":{"uid":data['info'][2][0], "name":data['info'][2][1], "is_admin":bool(data['info'][2][2]), "is_vip":bool(data['info'][2][3])}}
                        print(f"[ 彈幕訊息 ] {danmu_msg}\n{data}")
                    case 'ENTRY_EFFECT':
                        effect = {data['data']}
                        print(f"[ 特效 ] {effect}")
                    case 'GUARD_BUY':
                        guard_msg = {"level":{data['data']['gift_name']}} # TODO
                    case 'HOT_RANK_CHANGED':
                        pass # TODO
                    case 'INTERACT_WORD':
                        interact_msg = {"act":"關注"if data['data']['msg_type'] == 2 else "進場", "time":data['data']['timestamp'], "trigger_by":{"uid":data['data']['uid'], "name":data['data']['uname']}}
